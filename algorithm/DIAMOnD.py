...
def DIAMOnD(G_original,seed_genes,max_number_of_added_nodes,alpha):
    ...
    for DIAMOnD_node_info in added_nodes:
        ...
        DIAMOnD_name = DIAMOnD_node_info[0]
        p = float(DIAMOnD_node_info[3])

        ranked_list.append([DIAMOnD_name,p])
...
class DIAMOnDAlgirithm(Algorithm):
    def __init__(self,source_genes,graph,algorithm_params):
        Algorithm.__init__(self, source_genes,graph,algorithm_params)

        self.max_number_added_nodes = self.algorithm_params['max_num_added_nodes']

    def run(self):
        added_node = DIAMOnD(self.G.ppi_network, self.source_node_list, max_number_of_added_nodes=self.max_number_added_nodes, alpha=1)
        return added_node
